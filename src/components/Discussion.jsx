  ...
  return (
    <div className="discussion__page w-full flex flex-col justify-start items-center absolute h-full" style={{position:`absolute`,top:`90px`}}>
      ...
      { showmention && 
        ...
        <li className='flex items-center justify-center gap-2 mt-1'> <input type="text" className='rounded-md h-8' value={presentmention} onChange={(e)=>{
        ...
        }}>X</div></li>
      {
        mentions.map((m)=>{
          ...
          {
          return(
      <li className='flex gap-1 items-start justify-center border-b-2 cursor-pointer px-2 py-2' onClick={()=>{console.log(m.name);
        setmention([...mention,m.id]);
        setNewMessage(newMessage + m.name);
        setpresentmention('');
        setshowmention(false);
        }}> <img src={m?.photo} alt="Profile" className='h-8 w-8 rounded-full'/> <div>{m?.name}</div></li>
          )
          }
        })
      }
